import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule, ReactiveFormsModule } from '@angular/forms';
import { MatTableModule } from '@angular/material/table';
import { MatPaginatorModule } from '@angular/material/paginator';
import { MatFormFieldModule } from '@angular/material/form-field';
import { MatInputModule } from '@angular/material/input';
import { MatSnackBarModule } from '@angular/material/snack-bar';
import { MatButtonModule } from '@angular/material/button';
import { MatIconModule } from '@angular/material/icon';
import { MatSelectModule } from '@angular/material/select';
import { MatDialogModule } from '@angular/material/dialog';
import { MatSortModule } from '@angular/material/sort';
import { MatToolbarModule } from '@angular/material/toolbar';
import { MatDatepickerModule } from '@angular/material/datepicker';
import { MatMenuModule } from '@angular/material/menu';
import { MatTabsModule } from '@angular/material/tabs';
import { MatCheckboxModule } from '@angular/material/checkbox';
import { MaterialFileInputModule } from 'ngx-material-file-input';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';
import { AllSessionsComponent } from './all-sessions/all-sessions.component';
import { ShowSessionComponent } from './show-session/show-session.component';
import { FormDialogComponent } from './all-sessions/dialog/form-dialog/form-dialog.component';
import { DeleteFormComponent } from './all-sessions/dialog/delete-form/delete-form.component';
import { SessionService } from './all-sessions/session.service';
import { SessionRoutingModule } from 'src/app/session/session-routing.module';
import { AddSessionComponent } from './add-session/add-session.component';
import { MatNativeDateModule } from '@angular/material/core';




@NgModule({
  declarations: [
    AllSessionsComponent,
    ShowSessionComponent,
    FormDialogComponent,
    DeleteFormComponent,
    AddSessionComponent
  ],
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
    MatTableModule,
    MatPaginatorModule,
    MatFormFieldModule,
    MatInputModule,
    MatSnackBarModule,
    MatButtonModule,
    MatIconModule,
    MatDialogModule,
    MatSortModule,
    MatToolbarModule,
    MatSelectModule,
    MatCheckboxModule,
    MatTabsModule,
    MatDatepickerModule,
    MatMenuModule,
    MaterialFileInputModule,
    MatProgressSpinnerModule,
    SessionRoutingModule,
    MatNativeDateModule,

  ],
  providers: [SessionService],
})
export class SessionModule { }
